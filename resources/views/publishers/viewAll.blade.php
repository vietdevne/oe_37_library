            ...
            <div class="input-group mb-3">
                <form action="{{ route('publishers.showAll') }}" method="GET" class="form-inline input-group">
                    <input name="search" type="text" class="form-control"
                        placeholder="{{ trans('main.publisher.search') }}">
        ...
        <div class="col-md-4">
            <div class="card mb-3">
                <div class="card-body text-center">
                    @if (file_exists( public_path().'/images/publishers/'.$publisher->pub_logo && $publisher->pub_logo !=
                    null ))
                        <img src="images/publishers/{{ $publisher->pub_logo }}" class="img-thumbnail rounded-circle h-50 w-50 mb-4">
                    @else
                        <img src="image/library.png" class="img-thumbnail rounded-circle h-50 w-50 mb-4">
                    @endif
                    <h5>
                        <a href="{{ route('publisher.detail', $publisher->pub_id ) }}">{{ $publisher->pub_name }}</a>
                    </h5>
